#ifndef _STM32F10X_GPIO_H//在头文件被多个文件调用时，为了防止重复定义
...
#define GPIO_Pin_8    ((uint16_t)0x0100)  /* 选择Pin8     (00000001 0000000)b */
...
#define GPIO_Pin_All ((uint16_t)0xFFFF)  /* 选择全部引脚 (11111111 11111111)b */
 ...
 typedef enum // GPIO输出速率枚举定义
 {
		GPIO_Speed_10MHz = 1, // 10MHZ (01)b，第一个值设置为1,下面的两个不需要再设置
 	  GPIO_Speed_2MHz, // 2MHZ (10)b
		GPIO_Speed_50MHz // 50MHZ (11)b
 } GPIOSpeed_TypeDef;
 
 typedef enum  // GPIO工作模式枚举定义
 {
...
void GPIO_SetBits(GPIO_TypeDef *GPIOx,uint16_t GPIO_Pin); //在头文件中将置位函数进行声明

void GPIO_ResetBits(GPIO_TypeDef *GPIOx,uint16_t GPIO_Pin);//在头文件中将复位函数进行声明
 
void GPIO_Init(GPIO_TypeDef* GPIOx, GPIO_InitTypeDef* GPIO_InitStruct); // GPIO初始化函数的声明

#endif _STM32F10X_GPIO_H  /*_STM32F10X_GPIO_H*/
